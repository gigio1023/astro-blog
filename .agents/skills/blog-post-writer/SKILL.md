...
사용자가 지적한 표현은 다음 작업부터 durable feedback으로 취급한다. 특히
`references/anti-patterns.md`의 "사용자 지적 slop" 항목은 초안 작성 후 반드시
검색해서 없앤다.
...
**개념적으로 친절하게 쓴다.** 짧게 쓰는 것과 설명을 생략하는 것은 다르다. 내부 프로젝트 용어, 로그명, 테스트 이름, 점수판식 숫자는 독자가 처음 본다고 가정한다. 먼저 "무엇을 확인한 것인지"를 한 문장으로 풀고, 필요할 때만 내부 이름을 뒤에 붙인다.
...
- 내부 프로젝트 용어를 설명 없이 던지지 마라. `14/14 live matrix`, `current-run evidence`, `stress run` 같은 표현은 독자에게 정보가 아니라 암호처럼 보인다. "작은 행동 14개를 각각 실행해 보고, 실제 inventory나 block 변화로 결과를 확인했다"처럼 개념을 먼저 쓴다.
