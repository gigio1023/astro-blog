...
- Hugo, Jekyll, Ghost all work as blog platforms, but in my setup, GSC sitemap registration only succeeded starting with Ghost.
...
I assumed it was a Hugo config issue at first. Checked `enableRobotsTXT = true`, ran the sitemap through XML validators. It passed, but the content had `favicon.ico` listed as a URL and inline SVG data URIs sneaking in. Fixed those. Went deeper — the HTTP response for sitemap.xml was coming back `304 Not Modified` with no `Content-Type` header. Added `.nojekyll` to stop GitHub Pages' Jekyll processing. Didn't help.

Moved to Cloudflare Pages. Deployment worked fine, sitemap registration still failed. Ran a control experiment with Jekyll — Ruby environment setup on macOS was its own ordeal (`Gem::FilePermissionError`, forgetting `eval "$(rbenv init - zsh)"` in `.zshrc`) — but got it deployed under the same conditions. Same failure. Confirmed the framework wasn't the issue.

## A domain didn't fix it either
...
Connected the domain, registered as a Domain property in GSC. Hugo + custom domain still didn't work. Jekyll + custom domain didn't work either. At this point I genuinely didn't know what was wrong.

Other people might have had zero issues with Hugo or Jekyll + a custom domain. There could have been a setting I missed, or a timing issue on GSC's end. In my setup, it just didn't work.

## Ghost fixed it

The sitemap issue resolved when I switched to Ghost. Ghost has sitemap, meta tags, and structured data built in — no configuration needed. Custom domain + Ghost + GSC Domain property worked immediately.

Why it failed with Hugo/Jekyll but worked with Ghost, I can't say precisely. Maybe Ghost handles SEO internals better, or maybe something changed on GSC's side between attempts. What's certain is the problem disappeared when I moved to Ghost.
...
I'd originally picked Ghost for the "writing experience," but the one doing the writing shifted from me to AI, and that criterion stopped mattering. Went back to Astro — what I was recommended in the first place. Markdown file-based, so agents work with it freely. Static site, no k8s. Deploy to Cloudflare Workers & Pages and done.
...
JSON-LD structured data — BlogPosting, BreadcrumbList, WebSite, and Person schemas applied by page type. Plus a series/subpost system, Mermaid diagrams, and KaTeX math rendering.
